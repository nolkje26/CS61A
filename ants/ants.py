    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in range(len(bees)):
                Insect.reduce_armor(bees[bee], amount + self.damage)
        else:
            for bee in range(len(bees)):
                Insect.reduce_armor(bees[bee], amount)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting:
            ...
            self.eaten_bee = rANTdom_else_none(self.place.bees)
            self.eat_bee(self.eaten_bee)
        # END Problem 6
